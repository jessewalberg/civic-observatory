...
function UsageCompactView({ stats }: { stats: UsageStats }) {
	const viewsRemaining =
		stats.summary_view.limit === Infinity
			? "Unlimited"
			: `${stats.summary_view.remaining} left`;

	const uploadsRemaining =
		stats.meeting_upload.limit === Infinity
			? "Unlimited"
	...
	return (
		<div className="py-2 px-3 space-y-1 border-t border-border">
			<div className="flex items-center justify-between">
				...
				<span
					className={`text-xs font-medium ${getColor(stats.summary_view.current, stats.summary_view.limit)}`}
				>
					{viewsRemaining}
				</span>
			</div>
			<div className="flex items-center justify-between">
				...
				<span
					className={`text-xs font-medium ${getColor(stats.meeting_upload.current, stats.meeting_upload.limit)}`}
				>
					{uploadsRemaining}
				</span>
			</div>
