...
export function configure(options: Partial<AuthKitConfig> = {}) {
	...
	config = {
		...
		cookiePassword:
			options.cookiePassword || getEnv("WORKOS_COOKIE_PASSWORD") || "",
		cookieName:
			options.cookieName || getEnv("WORKOS_COOKIE_NAME") || DEFAULTS.cookieName,
		cookieMaxAge: options.cookieMaxAge || DEFAULTS.cookieMaxAge,
		apiHostname:
			options.apiHostname ||
			getEnv("WORKOS_API_HOSTNAME") ||
			DEFAULTS.apiHostname,
...
export function getConfig<K extends keyof AuthKitConfig>(
	...
	if (!config) {
		throw new Error("AuthKit config not initialized");
	}

	const value = config[key];

	// Check required values
	const required: (keyof AuthKitConfig)[] = [
		"clientId",
		"apiKey",
		"redirectUri",
		"cookiePassword",
