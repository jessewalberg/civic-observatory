...
export const Route = createFileRoute('/dashboard/subscriptions')({
  ...
  head: () => ({
    meta: [
      ...
      { name: 'description', content: 'Manage your municipality subscriptions' },
    ],
  }),
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        ...
        >
          ...
          <p className="text-muted-foreground mb-6">
            Sign in to manage your subscriptions and get notified about new meeting summaries.
          </p>
          <a href={signInUrl}>
...
function SubscriptionsContent({ workosUserId }: { workosUserId: string }) {
  ...
  // Mutations
  const toggleActive = useMutation(api.functions.subscriptions.mutations.toggleActive)
  const removeSubscription = useMutation(api.functions.subscriptions.mutations.remove)
  ...
  const handleToggleActive = async (subscription: Subscription) => {
    try {
      ...
      toast.success(result.isActive ? 'Subscription activated' : 'Subscription paused')
    } catch (error) {
  ...
  const handleDelete = async (subscription: Subscription) => {
    if (!confirm(`Unsubscribe from ${subscription.municipality?.name ?? 'this municipality'}?`)) {
      return
    }
        ...
        >
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-muted-foreground">
                Manage your municipality subscriptions and notification preferences.
              </p>
            </div>
            <a href="/explore">
              <Button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
            ...
            <Card className="p-4 hidden sm:block">
              <p className="text-2xl font-bold text-foreground">
                {user.tier === 'pro' ? 'Unlimited' : `${5 - (subscriptionCount?.total ?? 0)}`}
              </p>
                    ...
                    <TableRow key={subscription._id}>
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {subscription.topicFilters && subscription.topicFilters.length > 0 ? (
                            subscription.topicFilters.slice(0, 2).map((topic) => (
                              <Badge key={topic} variant="secondary" className="text-xs">
                                {topic}
                              </Badge>
                            ))
                          ) : (
                            <span className="text-xs text-muted-foreground">All topics</span>
                          )}
                          {subscription.topicFilters && subscription.topicFilters.length > 2 && (
