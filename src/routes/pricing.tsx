...
const plans = [
  ...
  },
  {
    ...
    price: '$15',
    period: 'per month',
    description: 'For power users who need unlimited access and real-time alerts.',
    features: [
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Custom integrations, dedicated support, and volume pricing for organizations.
                  </p>
                </div>
              </div>
              ...
              <Accordion type="single" collapsible className="w-full">
                <AccordionItem value="change-plans" className="border-b px-4">
                  ...
                  <AccordionContent className="text-muted-foreground">
                    Yes! You can upgrade to Pro at any time, and your usage will be instantly unlocked. If you downgrade, your Pro features will remain active until the end of your billing period.
                  </AccordionContent>
                </AccordionItem>
                <AccordionItem value="payment-methods" className="border-b px-4">
                  <AccordionTrigger className="text-left">
                ...
                <AccordionItem value="free-trial" className="border-b px-4">
                  ...
                  <AccordionContent className="text-muted-foreground">
                    We don't offer a traditional free trial, but you can use our Free plan indefinitely to try out the platform. When you're ready for more, upgrade to Pro.
                  </AccordionContent>
                </AccordionItem>
                <AccordionItem value="exceed-limits" className="border-b-0 px-4">
                  <AccordionTrigger className="text-left">
                    What happens if I exceed my limits?
                  </AccordionTrigger>
                  <AccordionContent className="text-muted-foreground">
                    On the Free plan, you'll see a message asking you to wait until your limits reset or upgrade to Pro. We never charge you without your consent.
                  </AccordionContent>
                </AccordionItem>
              </Accordion>
            </Card>
  ...
  return (
    ...
    >
      <Card className="p-6 bg-primary/5 border-primary/20">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                You're on the <span className="text-foreground font-medium capitalize">{usageStats.tier}</span> plan
              </p>
            </div>
          </div>
          {!isPro && (
        ...
        {usageStats.summary_view.resetsAt && usageStats.summary_view.limit !== Infinity && (
          <p className="text-xs text-muted-foreground mt-3">
            Daily views reset {formatResetTime(usageStats.summary_view.resetsAt)}
          </p>
        )}
      </Card>
    </motion.div>
...
function PlanCard({ plan, isLoggedIn, signInUrl, workosUserId }: PlanCardProps) {
  ...
  const createCheckoutSession = useAction(api.functions.stripe.actions.createCheckoutSession)
  const createPortalSession = useAction(api.functions.stripe.actions.createPortalSession)
  ...
  return (
    ...
    >
      ...
      {isCurrentPlan ? (
        ...
        <a href={signInUrl}>
          <Button className={cn('w-full', !isPro && 'variant-outline')} variant={isPro ? 'default' : 'outline'}>
            Sign in to {plan.cta}
          </Button>
        </a>
      )}
...
function FeatureValue({
  ...
  if (typeof value === 'boolean') {
    return value ? (
      <Check className={cn('h-5 w-5 mx-auto', highlighted ? 'text-primary' : muted ? 'text-muted-foreground' : 'text-emerald-500')} />
    ) : (
      <X className="h-5 w-5 mx-auto text-muted-foreground/50" />
    )
  }
  return (
    <span className={cn('text-sm', highlighted ? 'text-primary font-medium' : muted ? 'text-muted-foreground' : 'text-foreground')}>
      {value}
    </span>
  )
}
