...
function ScrapersContent({ workosUserId }: { workosUserId: string }) {
  ...
  const failedJobs = useQuery(api.functions.scrapeJobs.queries.getFailed, { limit: 10 })
  const municipalities = useQuery(api.functions.municipalities.queries.list, {})

  // Actions
  const triggerScrape = useAction(api.functions.scrapeJobs.mutations.triggerScrape)

  const handleScrapeNow = async (municipalityId: Id<'municipalities'>, municipalityName: string) => {
  ...
  const activeMunicipalities = municipalities?.filter((m) => m.isActive && m.platform !== 'manual').length ?? 0
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {municipalities?.map((muni) => {
                        ...
                        const canScrape = muni.isActive && !isManual && muni.meetingsPageUrl
                        const isExpanded = expandedId === muni._id
                        const isScraping = scrapingIds.has(muni._id)

                        return (
                          <Collapsible key={muni._id} open={isExpanded} onOpenChange={(open) => setExpandedId(open ? muni._id : null)} asChild>
                            <>
                              <TableRow
                                className={cn(
                                  'cursor-pointer',
                                ...
                                <TableCell>
                                  <div className="flex items-center gap-2">
                                    ...
                                    {muni.lastScrapeStatus && (
                                      <StatusBadge status={muni.lastScrapeStatus} />
                                    )}
                                  </div>
                                </TableCell>
                                    ...
                                    <div className="bg-muted/20 border-t border-border p-4">
                                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                        <div>
                                          ...
                                          <p className="text-foreground">{muni.platform ?? 'Not set'}</p>
                                        </div>
                                        <div>
                                          <p className="text-muted-foreground mb-1">Last Scraped</p>
                                          <p className="text-foreground">
                                            {muni.lastScrapedAt
                                              ? new Date(muni.lastScrapedAt).toLocaleString()
                                              : 'Never'}
                                          </p>
                                        </div>
                                        <div>
                                          <p className="text-muted-foreground mb-1">Last Status</p>
                                          <p className="text-foreground">
                                        ...
                                        {muni.meetingsPageUrl && (
                                          <div className="col-span-2 md:col-span-4">
                                            <p className="text-muted-foreground mb-1">Meetings URL</p>
                                            <a
                    ...
                    <TableBody>
                      {runningJobs.map((job) => (
                        <TableRow key={job._id} className="bg-blue-500/5">
                          ...
                          <TableCell className="text-muted-foreground text-sm">
                            {job.runningFor ? formatDuration(job.runningFor) : 'Starting...'}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
