...
export const getUsage = query({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      used,
      limit: dayLimit,
      remaining: dayLimit === Infinity ? Infinity : Math.max(0, dayLimit - used),
      windowType: "day" as const,
    };
  },
});
  ...
  handler: async (ctx, args) => {
    ...
    if ((actionLimits as { day?: number }).day !== undefined) {
      ...
      if (dayLimit !== Infinity && used >= dayLimit) {
        return { allowed: false, remaining: 0, limit: dayLimit, windowType: "day" };
      }
    ...
    if (monthLimitValue !== undefined) {
      ...
      if (used >= monthLimitValue) {
        return { allowed: false, remaining: 0, limit: monthLimitValue, windowType: "month" };
      }
