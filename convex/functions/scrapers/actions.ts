...
export const runScraper = internalAction({
  ...
  handler: async (ctx, args): Promise<RunScraperResult> => {
    ...
    try {
      ...
      if (!scraper) {
        throw new Error(`No scraper available for platform: ${municipality.platform}`);
      }
      ...
      // 6. Run scrape
      const result = await scraper.scrape(municipality.meetingsPageUrl, scraperConfig);
        ...
        try {
          ...
          if (!rawContent && meeting.documentUrl) {
            ...
            if (!meeting.documentUrl.toLowerCase().includes(".pdf")) {
              rawContent =
                (await scraper.extractContent(meeting.documentUrl, scraperConfig)) ??
                undefined;
            }
          }
      ...
      const municipalityStatus = finalStatus === "completed" ? "success" : finalStatus;
      await ctx.runMutation(
