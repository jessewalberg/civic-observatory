...
export const genericScraper: Scraper = {
    ...
    try {
      ...
      if (!selectors.meetingList) {
        return {
          ...
          errors: [
            {
              message: "Could not find meeting list on page. Configure meetingListSelector.",
              url,
              code: "parse",
              timestamp: Date.now(),
            },
      ...
      meetingElements.each((_, node) => {
        try {
          ...
          stats.failed++;
          errors.push({
            message: error instanceof Error ? error.message : "Failed to extract meeting",
            url,
            code: "parse",
            timestamp: Date.now(),
          });
  ...
  async extractContent(url: string, config?: ScraperConfig): Promise<string | null> {
    try {
      ...
      $("header, footer, nav, aside, script, style, .sidebar, .menu, .navigation").remove();
...
function getSelectors(
  ...
  if (config?.meetingListSelector) {
    return {
      ...
      meetingTitle: config.meetingLinkSelector || DEFAULT_SELECTORS.meetingTitle[0],
